...
Future<void> Function(Duration position)? _externalSeekHandler;
Future<void> Function()? _externalNextHandler;
...
class JustAudioBackground {
  static void setSeekHandler(
      Future<void> Function(Duration position)? handler) {
    _externalSeekHandler = handler;
  }

  static void setChapterNavigationHandlers({
    Future<void> Function()? onNext,
    Future<void> Function()? onPrevious,
    bool Function()? hasNext,
    bool Function()? hasPrevious,
  }) {
    _externalNextHandler = onNext;
...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  Duration _displayClockPosition = Duration.zero;
  DateTime _displayClockUpdatedAt = DateTime.now();
  Duration? _displayClockOffset;
  ...
  bool get canSkipToNext => hasNext || _externalBool(_externalHasNextHandler);
  bool get canSkipToPrevious =>
      hasPrevious || _externalBool(_externalHasPreviousHandler);

  bool _externalBool(bool Function()? callback) {
  ...
  Future<void> skipToNext() async {
    ...
    if (_externalBool(_externalHasNextHandler)) {
      await _externalNextHandler?.call();
    }
  }
  ...
  Future<void> skipToPrevious() async {
    ...
    if (_externalBool(_externalHasPreviousHandler)) {
      await _externalPreviousHandler?.call();
    }
  }
  ...
  void _updatePosition() {
    if (_mediaOffset > Duration.zero) {
      ...
      _justAudioEvent = _justAudioEvent.copyWith(
        updatePosition: _sourcePosition(displayPosition),
        updateTime: DateTime.now(),
      );
      return;
    }
  ...
  Duration get _displayPosition {
      ...
      if (_playing &&
        ...
        position += Duration(
          milliseconds: (DateTime.now()
                      .difference(_displayClockUpdatedAt)
                      .inMilliseconds *
                  _speed)
              .round(),
        );
    ...
    final position = currentPosition + _mediaOffset;
    final duration = currentMediaItem?.duration;
    if (duration != null && position > duration) return duration;
    return position;
  }

  void _ensureDisplayClock(Duration offset) {
    if (_displayClockOffset == offset) return;
    _resetDisplayClock(offset);
  }

  void _resetDisplayClock(Duration position) {
    _displayClockPosition = position;
    _displayClockUpdatedAt = DateTime.now();
    _displayClockOffset = _mediaOffset;
  }

  Duration _sourcePosition(Duration position) {
    final offset = _mediaOffset;
    if (offset == Duration.zero) return position;
    final sourcePosition = position - offset;
    return sourcePosition < Duration.zero ? Duration.zero : sourcePosition;
  }

  @override
  Future<void> seek(Duration position) async {
    final handler = _externalSeekHandler;
    if (handler != null) {
      await handler(position);
      _resetDisplayClock(position);
      _justAudioEvent = _justAudioEvent.copyWith(
        updatePosition: _sourcePosition(position),
        updateTime: DateTime.now(),
      );
      _broadcastState();
      return;
    ...
    await (await _player).seek(
      SeekRequest(position: _sourcePosition(position)),
    );
  }
  ...
  Future<void> _seekRelative(Duration offset) async {
    var newPosition = _displayPosition + offset;
  ...
  void _broadcastState() {
    ...
    final playPauseAction = _playing ? MediaAction.pause : MediaAction.play;
    ...
    playbackState.add(playbackState.nvalue!.copyWith(
      ...
      androidCompactActionIndices: compactActions
          .map(
            (action) =>
                controls.indexWhere((control) => control.action == action),
          )
          .where((index) => index >= 0)
          .toList(),
      ...
      playing: _playing,
      updatePosition: _displayPosition,
...
class _Seeker {
  ...
  Future<void> start() async {
    _running = true;
    while (_running) {
      Duration newPosition = handler._displayPosition + positionInterval;
      if (newPosition < Duration.zero) newPosition = Duration.zero;
