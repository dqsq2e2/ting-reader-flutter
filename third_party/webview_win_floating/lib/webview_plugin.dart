...
class WindowsPlatformWebViewCookieManager extends PlatformWebViewCookieManager {
  ...
  static const MethodChannel _cookieChannel =
      MethodChannel('webview_win_floating');
  ...
  Future<void> setCookie(WebViewCookie cookie) async {
    final result = await _cookieChannel.invokeMethod<bool>('setCookie', {
      'name': cookie.name,
      'value': cookie.value,
    ...
    if (result != true) {
      throw StateError('Windows WebView2 failed to write cookie');
    }
  }

  @override
  Future<List<WebViewCookie>> getCookies(Uri url) async {
    final result = await _cookieChannel.invokeMethod<List<dynamic>>(
      'getCookies',
      {
        'url': url.toString(),
      },
