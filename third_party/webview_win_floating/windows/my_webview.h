...
using WebViewCookieCallback =
    std::function<void(HRESULT, std::vector<WebViewCookieData>)>;
...
public:
	...
	virtual HRESULT getCookies(LPCWSTR url, WebViewCookieCallback callback) = 0;
	virtual HRESULT setCookie(LPCWSTR name, LPCWSTR value, LPCWSTR domain,
	                          LPCWSTR path) = 0;
