...
void WebviewWinFloatingPlugin::HandleMethodCall(
  ...
  if (method_call.method_name().compare("getCookies") == 0 ||
    ...
    if (webviewIdIt != arguments.end()) {
      ...
      if (webviewIt != m_webviewMap.end()) webview = webviewIt->second;
    }
    if (webview == nullptr && !m_webviewMap.empty()) {
      webview = m_webviewMap.rbegin()->second;
    }
    if (webview == nullptr) {
      ...
      auto hr = webview->getCookies(
          ...
          [shared_result](HRESULT callbackResult,
            ...
            if (FAILED(callbackResult)) {
              shared_result->Error("getCookies failed", std::to_string(callbackResult));
              return;
            }

            flutter::EncodableList encodedCookies;
            for (const auto& cookie : cookies) {
              flutter::EncodableMap encodedCookie;
              encodedCookie[flutter::EncodableValue("name")] =
                  flutter::EncodableValue(cookie.name);
              encodedCookie[flutter::EncodableValue("value")] =
                  flutter::EncodableValue(cookie.value);
              encodedCookie[flutter::EncodableValue("domain")] =
                  flutter::EncodableValue(cookie.domain);
              encodedCookie[flutter::EncodableValue("path")] =
                  flutter::EncodableValue(cookie.path);
              encodedCookies.emplace_back(encodedCookie);
            }
            shared_result->Success(flutter::EncodableValue(encodedCookies));
          });
      if (FAILED(hr)) {
        shared_result->Error("getCookies failed", std::to_string(hr));
      }
      return;
    }
